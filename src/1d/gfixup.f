...
c
!$OMP PARALLEL DO 
!$OMP&            PRIVATE(j,mptr,nx,mitot,corn1,loc)
!$OMP&            PRIVATE(locaux,time,mic,xl,xr,ilo,ihi)
!$OMP&            PRIVATE(sp_over_h,thisSetauxTime)
!$OMP&            SHARED(newnumgrids,listnewgrids,nghost,node,hx)
!$OMP&            SHARED(rnode,intratx,lcheck,nvar,alloc,naux)
...
c
 ...
 85   if (mptr .eq. 0) go to 90
          nx = node(ndihi,mptr) - node(ndilo,mptr) + 1
          mitot = nx + 2*nghost
          call reclam(node(store1,mptr),mitot*nvar)
          if (naux .gt. 0) then
