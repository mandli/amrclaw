...
c ---------------------------------------------------------
c
      logical function nestck1(mnew,lbase,badpts,npts,numptc,icl,
     1                        nclust, nvar,naux)
c
      use amr_module
...
c
 50    if (npts .gt. 1) go to 55
           ...
           write(*,101)       levnew
 101       format(' nestck1: 1 pt. cluster at level ',i5,' still not',
     1       ' nested',/,'          pt. too close to boundary')
           write(outunit,104) badpts(1,npts)
           write(*,104)       badpts(1,npts)
 104       format(' non-nested flagged pt. at: ',e15.7)
 ...
 55    if (nclust .lt. maxcl) go to 60
           ...
           write(*,102)       maxcl
 102       format(' too many clusters: > ',i5,' (from nestck1) ')
           stop
