...
import clawpack.amrclaw.data as amr_data
...
#------------------------------
    ...
    rundata.regiondata.regions.append([1, 1, 0, 1e10, 0.0, 1.0, 0.0, 1.0])
    rundata.regiondata.regions.append([1, 3, 0, 1e10, 0.0, 1.0, 0.0, 0.7])

    # ========================
    #  Aux File Specification
    # ========================
    # to specify aux file input append lines of the form
    rundata.auxdata.aux_files = []
    u_vel_file = amr_data.AuxFile("./u_velocity.tt3")
    u_vel_file.file_type = 3
    u_vel_file.init_type = 1
    rundata.auxdata.aux_files.append(u_vel_file)

    v_vel_file = amr_data.AuxFile("./v_velocity.tt2")
    v_vel_file.file_type = 2
    v_vel_file.init_type = 2
    rundata.auxdata.aux_files.append(v_vel_file)

    # Generate test aux file
    x = numpy.linspace(rundata.clawdata.lower[0],
                       rundata.clawdata.upper[0], 10)
    y = numpy.linspace(rundata.clawdata.lower[1],
                       rundata.clawdata.upper[1], 10)
    u_velocity = topotools.Topography(
                          topo_func=lambda x, y: numpy.cos(2.0 * numpy.pi * y))
    u_velocity.x = x
    u_velocity.y = y
    u_velocity.write("u_velocity.tt3", topo_type=3)
    v_velocity = topotools.Topography(
                          topo_func=lambda x, y: numpy.sin(2.0 * numpy.pi * x))
    v_velocity.x = x
    v_velocity.y = y
    v_velocity.write("v_velocity.tt2", topo_type=2)
